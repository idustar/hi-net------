    ...
    layers: [
      {
        ...
        padding: 0,
        activation: 'relu',
        desc: 'Convolution layer will compute the output of neurons that are connected to local regions in the input, each computing a dot product between their weights and a small region they are connected to in the input volume.\n\nactivation function: will apply an element-wise activation function, such as the max(0,x) thresholding at zero\nfilters: each learning to look for something different in the input.\nstride: the way slide the filter. When the stride is 1 then we move the filters one pixel at a time. When the stride is 2 then the filters jump 2 pixels at a time as we slide them around. This will produce smaller output volumes spatially.\npadding: pad the input volume with zeros around the border.\nkernel size: specify the length of the convolution window '
      },
      {
        type: 'dense',
        layerId: 1,
      ...
      },
      {
        ...
        strides: 2,
        poolingType: 'max',
        desc: 'POOL layer will perform a downsampling operation along the spatial dimensions (width, height), it progressively reduce the spatial size of the representation to reduce the amount of parameters and computation in the network, and hence to also control overfitting.\nkernel size:  size of the pooling windows\nstrides: downsampling factor\npooling type:\nmax: max pooling operation for spatial data'
      },
      {
        layerId: 3,
        type: 'dropout',
        ctype: 'Dropout Layer',
        dropoutRate: 0.5,
        desc: 'randomly setting a fraction rate of input units to 0 at each update during training time, which helps prevent overfitting.\n\ndropout rate: float between 0 and 1. Fraction of the input units to drop.',
      },
      {
        layerId: 4,
        ctype: 'Output Layer',
